...
/*
* Calculate the degree of a node according to the nature of the incoming node
*
* @param	i		index of the node
* @param	a		adjacency matrix
* @param	n	 	number total of node
...
*/
int nodeDegreeType(int i, int **a, int *type_neuron, int type_selected, int n)
{
	int j, degree = 0;

	for(j=0; j<n; j++)		//for each nodes
	{
		if((a[i][j]==1) && (type_neuron[j]==type_selected))		//if there is a link
		{
			degree++;
		}
	}
